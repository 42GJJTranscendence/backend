...
export class AuthService {
  ...
  async checkVerificationCode(email: string, code: string): Promise<Boolean> {
    ...
    if (codeFind == null || codeFind != code) return false;
    else {
      ...
      .then(async (response) => {
        if (!response.ok) {
          return Promise.reject(
            `Error ${response.status}: Failed to get user infos`,
          );
        }
  ...
  async signInUser(
    ...
    const payload: Payload = {
      ...
      fortyTwoId: createdUser.fortyTwoId,
    };
  ...
  async validateUser(
    ...
    let userFind: User = await this.userService.findOneByUsername(
    ...
    const payload: Payload = {
      ...
      fortyTwoId: userFind.fortyTwoId,
    };
